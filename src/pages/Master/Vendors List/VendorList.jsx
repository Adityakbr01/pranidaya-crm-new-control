...
import { Users, Phone, Mail, FileText, CheckCircle2, ShieldCheck } from "lucide-react";
...
const VendorList = () => {
  ...
  const stats = useMemo(() => {
    const total = overdueListData.length;
    const active = overdueListData.filter(
      (v) => (v.vendor_status || "").toLowerCase() === "active"
    ).length;
    const withGst = overdueListData.filter(
      (v) => v.vendor_gst && v.vendor_gst.trim() !== ""
    ).length;
    return { total, active, withGst };
  }, [overdueListData]);
      ...
      options: {
        ...
        customBodyRender: (value) =>
          value ? (
            ...
            <span className="text-slate-400 dark:text-slate-500 italic text-xs">Unregistered</span>
          ),
      },
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value) =>
          value ? (
            <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300 text-xs tabular-nums font-medium">
      ...
      options: {
        ...
        customBodyRender: (id) => (
          <div className="flex items-center space-x-2">
            <Tooltip title="Edit Vendor Details" arrow>
              <div>
                <EditVendorItem
                  onClick={() => {
            ...
            title={
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-slate-900 dark:text-slate-100">
                  Registered Vendors & Supplier Directory
                </span>
              </div>
            }
