...
import { AddAnimal, EditAnimal } from "@/components/ButtonComponents.jsx";
...
import { useAnimalTypeList } from "@/modules/AnimalStock";
import { Tag, CheckCircle2, AlertCircle } from "lucide-react";
import { Tooltip } from "@mui/material";

const Animal = () => {
  const navigate = useNavigate();
  const { data: animalList = [], isLoading, isError } = useAnimalTypeList();

  const stats = useMemo(() => {
    const total = animalList.length;
    const active = animalList.filter(
      (a) => (a.animal_type_status || "").toLowerCase() === "active"
    ).length;
      ...
      options: {
        ...
        customBodyRender: (value) => (
          <div className="flex items-center gap-2.5 py-1">
            <div className="w-8 h-8 rounded-lg bg-teal-50 dark:bg-teal-950/60 border border-teal-100 dark:border-teal-800/60 flex items-center justify-center text-teal-600 dark:text-teal-400 shrink-0">
      ...
      options: {
        ...
        customBodyRender: (id) => (
          <div className="flex items-center space-x-2">
            <Tooltip title="Edit Animal Category" arrow>
              <div>
                <EditAnimal
                  onClick={() => {
                    const encryptedId = encryptId(id);
                    navigate(`/edit-animal/${encodeURIComponent(encryptedId)}`);
                  }}
  ...
  return (
    <Layout>
      ...
      {isLoading ? (
        ...
        <div className="text-rose-500 text-center mt-4">Error loading animal data</div>
      ) : (
            ...
            title={
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-slate-900 dark:text-slate-100">
                  Animal Types & Breeds Master Register
                </span>
              </div>
            }
            data={animalList}
